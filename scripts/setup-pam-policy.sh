...
grep -q "pam_pwquality.so" /etc/pam.d/common-password || {
    echo "[WARN] pam_pwquality not found in common-password"
    errors=$((errors + 1))
}
grep -q "remember=5" /etc/pam.d/common-password || {
    echo "[WARN] password history not set in common-password"
    errors=$((errors + 1))
}
grep -q "pam_faillock.so" /etc/pam.d/common-auth || {
    echo "[WARN] pam_faillock not found in common-auth"
    errors=$((errors + 1))
}
