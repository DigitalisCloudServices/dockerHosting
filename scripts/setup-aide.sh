...
if [[ "$FORCE" == false ]] && [[ -f /etc/aide/aide.conf ]] && grep -q "Generated by dockerHosting" /etc/aide/aide.conf 2> /dev/null; then
...
# Database paths (AIDE 0.18+ rejects the old 'database=' keyword)
...
# growing+s: size may only grow, md5: MD5 checksum, sha256: SHA256 checksum
...
Logs = p+u+g+i+n+growing+s

# Databases (size can grow)
Databases = p+u+g+i+n+growing+s
