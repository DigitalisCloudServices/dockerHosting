...
Traefik replaces system-level nginx as the boundary reverse proxy. It runs as a Docker container and hot-reloads *dynamic* routing config from files — no restart required. The *static* config (`/etc/traefik/traefik.yml`) is different: Traefik only reads it at startup, so a static-config change needs a container restart to take effect (see [Idempotency and config drift](#idempotency-and-config-drift) below).

### Cloudflare-trusted forwarded headers

Production traffic flows Cloudflare → Traefik → Kong. `templates/traefik/traefik.yml` sets `forwardedHeaders.trustedIPs` on both the `web` and `websecure` entryPoints, populated from `templates/traefik/cloudflare-ips.txt` (one CIDR per line, `#` comments and blank lines ignored) at `write_configs` time in `scripts/install-traefik.sh`. This is what makes Traefik trust the Cloudflare edge as a proxy and pass through Cloudflare's `X-Forwarded-For` / `X-Forwarded-Proto` instead of discarding them and substituting the edge's own IP.

Without this, every request Kong sees has a Cloudflare PoP address (`141.101.x`, `162.158.x`, `172.64-172.71.x`, …) as its client IP, never the real visitor — so per-IP rate limiting counts per Cloudflare PoP instead of per user.

Re-vendor the IP list periodically (Cloudflare changes these ranges infrequently, without notice) from <https://www.cloudflare.com/ips-v4> and <https://www.cloudflare.com/ips-v6>, update the header comment's vendored date, and re-run `install-traefik.sh` — the drift check below picks up the change automatically.

**Ordering with Kong:** Kong's `KONG_TRUSTED_IPS=0.0.0.0/0` with recursive real-IP resolution takes the *leftmost* entry of `X-Forwarded-For`. Once Traefik passes that header through, it is attacker-controlled unless Kong is also configured to trust only the Traefik hop and take the correct entry. The Kong-side fix ships separately (VelaAir); it must land before or together with this change, or client IPs become spoofable.
...
### Idempotency and config drift

`install-traefik.sh` is idempotent: if a `traefik:<version>`-tagged container is already running, a plain re-run is a no-op. That check alone used to be too coarse — a static-config change (e.g. re-vendoring the Cloudflare IP list) would never deploy to an already-running host, because the image tag matched and the script exited before writing anything.

The installer now also compares the deployed `/etc/traefik/traefik.yml` against what the current template + `cloudflare-ips.txt` would render (`config_drifted` in `scripts/install-traefik.sh`). Three outcomes:

| State | Result |
|---|---|
| Same image tag, config matches | Skip (as before) |
| Same image tag, config **drifted** | Reinstall automatically — non-interactive, no firewall prompt, existing dashboard credentials preserved |
| `--force` | Always reinstalls (existing behaviour — interactive firewall prompt, dashboard password rotated) |

A drift-triggered reinstall re-renders the static config, restarts the Traefik container so it picks up the change, and skips both the nginx-migration prompt and the port-8080 firewall prompt (`prompt_firewall_8080`) so an unattended run never blocks on `read`. It also does **not** rotate the dashboard password — `write_configs` reuses the password already on disk at `/etc/traefik/dashboard-credentials` instead of calling `generate_dashboard_password`. Manual `--force` is unchanged: it always regenerates the dashboard password and prompts interactively, as before.
...
│   ├── traefik/
│   │   ├── traefik.yml               # Traefik static config template (Cloudflare trustedIPs marker)
│   │   ├── cloudflare-ips.txt        # Cloudflare edge CIDRs, spliced into traefik.yml at install time
...
| `tests/traefik/test_install_traefik.bats` | 53 tests: nginx detection, migration, config writing, Cloudflare trustedIPs rendering, deploy-drift reinstall |
...
| `tests/traefik/test_remove_site.bats` | 13 tests: removal, site listing, domain-to-name conversion |
| `tests/traefik/test_install_traefik.bats` | 53 tests: nginx detection, migration helpers, config writing, docker invocations, prompt flags, Cloudflare trustedIPs rendering, deploy-drift reinstall |
